...
var Account, JsSIPAccount, OktellVoice, SIPml5Account, debugMode, eventSplitter, events, extend, key, log, logErr, logStr, manager, _i, _len, _ref,
  __slice = [].slice,
...
for (_i = 0, _len = _ref.length; _i < _len; _i++) {
  key = _ref[_i];
  OktellVoice[key] = function() {
    return false;
  };
}

OktellVoice.connect = function() {
  var acc, exportAcc,
    _this = this;
  ...
  if (acc === manager.defaultAcc) {
    ...
    exportAcc.on('all', function() {
      var args;

      args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      return OktellVoice.trigger.apply(OktellVoice, args);
    });
    OktellVoice.on('all', function() {
      var args, eventname;
...
OktellVoice.disconnect = function() {};
